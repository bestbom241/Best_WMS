<template>
  <div class="container">
    <div class="form-card">
      <h2>+ New Outbound Plan</h2>
      <div class="form-group">
        <label>Customer Code</label>
        <select v-model="form.customer_code">
          <option value="">-- เลือก Customer --</option>
          <option v-for="cust in customers" :key="cust.id" :value="cust.customer_code">
    ...
    <div class="list-card">
      <h2>Outbound Plan (ส่งของออก)</h2>
      <button @click="fetchPlans">Refresh</button>
      <table v-if="plans.length > 0">
        <thead>
          <tr>
            <th>Plan Code</th><th>Customer</th><th>SKU</th><th>Plan Qty</th><th>Picked</th><th>Plan Date</th><th>Status</th>
          </tr>
        </thead>
        <tbody>
...
const form = ref({ customer_code: '', sku: '', plan_qty: 0, plan_date: '' })
...
const createPlan = async () => {
  ...
  message.value = ''
  try {
    await axios.post('/api/outbound-plans', form.value, { headers: authHeader() })
    message.value = 'สร้าง plan สำเร็จ!'
    success.value = true
    form.value = { customer_code: '', sku: '', plan_qty: 0, plan_date: '' }
    fetchPlans()
  } catch (err) {
